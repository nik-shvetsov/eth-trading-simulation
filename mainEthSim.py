...
class Simulation(object):
    def __init__(self, sp, simdays, typeAuc, eTokenContract, eMarketContract):
        ...
        self.graph_data_batteryBalance = defaultdict(
            list
    ...
    def checkCoinBalance(self, address):
        print(
            "Coin balance of {0} is {1}".format(
                address, self.eTokenC.call().balanceOf(address)
            )
        )
    ...
    def checkEnergyBalance(self, address):
        print(
            "Energy balance of {0} is {1}".format(
                address, self.eMarketC.call({"from": address}).getEnergyBalance()
            )
        )
    ...
    def buyCoins(self, receiver, amount):
        ...
        txhash = self.eMarketC.transact().transactMoney(
            receiver, self.sp.address, amount * buy_rate
        )
        check = chain.wait.for_receipt(txhash)

        txhash = self.eMarketC.transact().sendCoin(
            self.eTokenC.address, self.sp.address, receiver, amount
        )
        check = chain.wait.for_receipt(txhash)
        # -------
    ...
    def sellCoins(self, seller, amount):
        ...
        txhash = self.eMarketC.transact().transactMoney(
            self.sp.address, seller, amount * sell_rate
        )
        check = chain.wait.for_receipt(txhash)

        txhash = self.eMarketC.transact().sendCoin(
            self.eTokenC.address, seller, self.sp.address, amount
        )
        check = chain.wait.for_receipt(txhash)
    ...
    def sendCoins(self, sender, receiver, numcoins):
        txhash = self.eMarketC.transact().sendCoin(
            self.eTokenC.address, sender, receiver, numcoins
        )
        check = chain.wait.for_receipt(txhash)
        # for x in self.sp.households:
        # if x.address == receiver:
    ...
    def buyEnergy(self, buyer, seller, energy, rate):
        txhash = self.eMarketC.transact({"from": buyer}).buyEnergy(
            self.eTokenC.address, seller, energy, rate
        )
        chain.wait.for_receipt(txhash)
        # -------------
        # for x in self.sp.households:
    ...
    def run(self):
        ...
        while self.current_day <= self.days_to_sim:
            ...
            while self.hour <= 23:  # 23
                ...
                # Auction process
                print(
                    "Application list before auction ['buy/sell', needValueToBuy, priceBuyRate]"
                )
...
def generateHouses(number, addressList, rulers):
    ...
    for i in range(1, number + 1):
        ...
        if houseInst.pv:
            rnd.seed()
            houseInst.ruler.pvSq = 1 * rnd.randint(
                1, 3
...
with project.get_chain("testrpc") as chain:
    ...
    buildGraph(
        hhbuild,
        sim.graph_data_production,
        "Production, Wh",
        "Production profiles",
        True,
    )
    buildGraph(
        hhbuild, sim.graph_data_coins, "Energy Coins, coins", "Energy coins", True
    )
    buildGraph(
        hhbuild, sim.graph_data_money, "Money Balance, cents", "Money balance", True
    )
    ...
    print(
        "Carbon energy print: ",
        round(
            sp.totalOuterGrid / float(sp.totalOuterGrid + sp.totalSolarEnergy) * 100, 2
        ),
        " %",
    )
    print(
        "Solar energy print: ",
        round(
            sp.totalSolarEnergy / float(sp.totalOuterGrid + sp.totalSolarEnergy) * 100,
            2,
        ),
        " %",
    )
