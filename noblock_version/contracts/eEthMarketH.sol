...
contract eToken {
    ...
    uint256 public sellPrice = 11; // 11 ether(cents) for token - 100 wh - 0.1 kwh
    uint256 public buyPrice = 9; // 9 ether(cents) for token - 100 wh - 0.1 kwh
...
contract EthEnergyMarketH {
    ...
    function buyFromOuterGrid(address account, uint Wh) onlyOwner {
        ...
        var money = int(Wh * (outer_rate));//sell using increased rate
        moneyBalance[account] -= money; //check convert
    }
    
    function sellToOuterGrid(address account, uint Wh) onlyOwner{
        if ( Wh > energyBalance[account] ) throw;
        energyBalance[account] -= Wh;
        var money = int(Wh * (rate)); //sell using usual rate
