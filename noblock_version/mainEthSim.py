...
import seaborn
import sp
from utils import buildGraph, buildSmoothXY, generateAdresses

warnings.filterwarnings("ignore")


class Simulation(object):
    def __init__(self, sp, simdays, typeAuc):
        self.sp = sp
        # self.houseList = None
        # self.rulerList = None
        ...
        # weatherType = {'sun': (1.5), 'rain': (0.2), 'clowds': (1.0)}
        ...
        self.graph_data_batteryBalance = defaultdict(list)
        # data for graphs # {address:consumption_time_print}
    ...
    def setInitialConditions(self, energy, tokens):
        for x in self.sp.households:
            x.ruler.batteryBalance = energy
    ...
    def priceFormationSell(self, amountMidRange, amount, qlearn, sample):
        if not qlearn:
            ...
            for x in self.sp.Qtable.keys():
                self.sp.QtableCluster.append([x[0][0], x[0][1], x[0][2], x[1][0]])
            # without price x[1][1]
            ...
            while finPrice == 0:
                ...
                for x in self.sp.QtableCluster:
                    euc = math.sqrt(
                        0.5 * pow(x[0] - sample[0], 2)
                        + 0.5 * pow(x[1] - sample[1], 2)
                        + 0.4 * pow(x[2] - sample[2], 2)
                        + 1.0 * pow(x[3] - sample[3], 2)
                    )
    ...
    def run(self):
        ...
        while self.current_day <= self.days_to_sim:
            ...
            while self.hour <= 23:  # 23
                ...
                for x in self.sp.households:
                    consumption[x.address] = x.ruler.getNextHourConsumption(
                        self.hour, hourWeather
                    )
                    production[x.address] = x.ruler.getLastHourProduction(
                        self.hour, hourWeather
                    )
                    ...
                    if len(x.ruler.avgConsRateList) > self.avgWindow:
                        ...
                        avgCons[x.address] = (
                            float(sum(x.ruler.avgConsRateList)) / self.avgWindow
                        )
                    else:
                ...
                for x in self.sp.households:
                    if (
                        production[x.address] + x.ruler.batteryBalance
                        > x.ruler.batteryCap
                    ):
                        self.sellToOuterGrid(
                            x.address,
                            production[x.address]
                            + x.ruler.batteryBalance
                            - x.ruler.batteryCap,
                        )
                ...
                # calculate need and action to do (using battery info,
                # (prod - cons), weather, profile, away/home(hour))
                # 4. Calculate future expences, determine action to do
                for x in self.sp.households:
                    balanceAfterConsumption = (
                        x.ruler.batteryBalance - consumption[x.address]
                    )
                    # print ("Balance after consumption, before auction: ",
                    # balanceAfterConsumption)
                    ...
                    if balanceAfterConsumption >= 0:
                        if (
                            balanceAfterConsumption
                            <= x.ruler.batteryCap * self.batteryTH
                        ):  # *0.1
                            # print ("Comparison: ", balanceAfterConsumption,
                            # "<=", x.ruler.batteryCap * self.batteryTH)
                            applicationList.pop(x.address)
                            # print ("Removed application, go to next hour")
                            # go to next hour / no need to buy/sell

                        else:
                            # balanceAfterConsumption > x.ruler.batteryCap*0.1:
                            # print ("Comparison: ", balanceAfterConsumption,
                            # ">", x.ruler.batteryCap * self.batteryTH)
                            ...
                            sample = [
                                self.hour,
                                round(x.ruler.batteryBalance / x.ruler.batteryCap, 3),
                                avgCons[x.address],
                                0,
                            ]
                            priceSellRate = self.priceFormationSell(
                                self.formMidRange(),
                                possibleAmountToSell,
                                qlearnTime,
                                sample,
                    ...
                    #  go buy (from auction or outer grid)
                        ...
                        # balanceAfterConsumption < 0
                        # print ("Comparison: ", balanceAfterConsumption, "<", 0)
                        # print ("Need to buy energy")
                        needValueToBuy = self.roundToWh(
                            math.fabs(balanceAfterConsumption)
                        )
                        ...
                        priceBuyRate = self.priceFormationBuy(
                            self.formMidRange(), needValueToBuy, qlearnTime, sample
                        )
                ...
                # Auction process
                print(
                    "Application list before auction ['buy/sell',\
                                                        needValueToBuy,\
                                                        priceBuyRate]"
                )
                ...
                if self.auctionType == "loyal":
                    ...
                    if len(buyListL) and len(sellListL) != 0:
                        ...
                        # [returnBuyList, returnSellList, cutSellList,
                        # cutBuyList, transactionsSummed]
                        ...
                        for x in retList[4]:
                            tempList.append(x)
                        if len(tempList) != 0:
                            self.sp.approvedTransactions[
                                self.sp.currentTimeStamp
                            ] = tempList
                            ...
                            for y in self.sp.households:
                                if y.address == x[0]:
                                    ...
                                    state = (
                                        self.hour,
                                        round(
                                            y.ruler.batteryBalance / y.ruler.batteryCap,
                                            3,
                                        ),
                                        consumption[x[0]],
                                    )
                                    ...
                                    key = (state, action)
                                    self.sp.Qtable[key] = (
                                        self.sp.Qtable[key] * y.loyalty + 0.1
                                    )
                        ...
                        if len(retList[0]) == 0:  # returnBuyList
                            ...
                            # evaluate returnBuyList - empty if all approved
                            # have got their needed energy
                            pass
                        else:
                            ...
                            for x in retList[0]:
                                for y in self.sp.households:
                                    if y.address == x[0]:
                                        ...
                                        key = (state, action)
                                        self.sp.Qtable[key] = (
                                            self.sp.Qtable[key] * y.loyalty + 0.05
                                        )

                        if len(retList[1]) == 0:  # returnSellList
                            # print("All approved amount sold!")
                            # evaluate returnSellList - empty if all
                            # approved amount is sold
                            pass
                        else:
                        ...
                        if len(retList[2]) == 0:  # cutSellList
                            # evaluate cutSellList - too high requests -
                            # rejected - need to low them - send this signal
                            pass
                        else:
                            ...
                            for x in retList[2]:
                                for y in self.sp.households:
                                    if y.address == x[0]:
                                        ...
                                        key = (state, action)
                                        self.sp.Qtable[key] = (
                                            self.sp.Qtable[key] * y.loyalty - 1
                                        )

                        if len(retList[3]) == 0:  # cutBuyList
                            # evaluate cutBuyList - too low bids - rejected -
                            # need to increase them - send this signal
                            pass
                        else:
                    ...
                    # execute additional transactions on outer grid if neeeded
                        ...
                        # no buys, sellers -> og selling needed
                        for x in sellListL:
                            for y in self.sp.households:
                                if x[0] == y.address:
                                    if (
                    ...
                    elif len(sellListL) == 0:
                        # no sells -> og buys needed
                        for x in buyListL:
                            self.buyFromOuterGrid(x[0], x[2])
                    ...
                    # updateInfoSProvider(households)
                    # -------------------------------------------

                    # add info to graph lists for this hour
                    for x in self.sp.households:
                        self.graph_data_batteryBalance[x.address].append(
                            x.ruler.batteryBalance
                        )
                        self.graph_data_consumption[x.address].append(
                            consumption[x.address]
                        )
                        self.graph_data_production[x.address].append(
                            production[x.address]
                        )
                        self.graph_data_coins[x.address].append(
                            self.getCoinBalance(x.address)
                        )
                        self.graph_data_money[x.address].append(
                            self.getMoneyBalance(x.address)
                        )

                        if len(self.graph_data_consumptionC[x.address]) != 0:
                            self.graph_data_consumptionC[x.address].append(
                                self.graph_data_consumptionC[x.address][-1]
                                + consumption[x.address]
                            )
                            self.graph_data_productionC[x.address].append(
                                self.graph_data_productionC[x.address][-1]
                                + production[x.address]
                            )
                            self.graph_data_coinsC[x.address].append(
                                self.getCoinBalance(x.address)
                                + self.graph_data_coinsC[x.address][-1]
                            )
                            self.graph_data_moneyC[x.address].append(
                                self.getMoneyBalance(x.address)
                                + self.graph_data_moneyC[x.address][-1]
                            )
                        else:
                            self.graph_data_consumptionC[x.address].append(
                                consumption[x.address]
                ...
                if self.auctionType == "simple":
                    ...
                    for x in applicationList:  # key: x - address
                        if applicationList[x][0] == "buy":
                            buyList.append(
                                [x, applicationList[x][1], applicationList[x][2]]
                            )
                        if applicationList[x][0] == "sell":
                            sellList.append(
                                [x, applicationList[x][1], applicationList[x][2]]
                    ...
                    if len(buyList) and len(sellList) != 0:
                        ...
                        if len(retList[1]) == 0:  # returnSellList
                            # print("All approved amount sold!")
                            # evaluate returnSellList - empty if all approved
                            # amount is sold
                            pass
                        else:
                            # perform action on outer grid
...
def generateHouses(number, addressList, rulers):
    ...
    for i in range(1, number + 1):
        ...
        if houseInst.pv:
            rnd.seed()
            houseInst.ruler.pvSq = 1 * rnd.randint(1, 3)
            # may be 3-4 times bigger PV panel
        else:
...
def setInitialRandConditions(households, energyMaxDiv, tokens):
    for x in households:
        rnd.seed()
...
# initialize network
init_addresses = generateAdresses(num_of_households)

accounts = init_addresses
addresses = list(accounts[1:])
spAccount = accounts[0]

addresses = list(addresses[:num_of_households])
# 9 agents
rulers = generateRulers(num_of_households, rulerCap)
# 9 households
households = generateHouses(num_of_households, addresses, rulers)
setInitialRandConditions(households, maxEnergyDiv, initCoins)
# setInitialConditions(households, maxEnergyDiv, initCoins)

sp = sp.ServiceProvider(0, spAccount, addresses, households)
print("Done initializing")

# sp, addresses, houses, rulers
sim = Simulation(sp, simDays, aucType)
sim.run()
# print("Simulation finished")
...
hhbuild = households[:]
buildGraph(
    hhbuild, sim.graph_data_batteryBalance, "Balance, Wh", "Battery balance", True
)
buildGraph(
    hhbuild, sim.graph_data_consumption, "Consumption, Wh", "Consumption profiles", True
)
buildGraph(
    hhbuild, sim.graph_data_production, "Production, Wh", "Production profiles", True
)
...
plt.plot(x, y, "--ro")
plt.savefig("Prices distribution")
plt.show()
...
plt.savefig("Auction evaluation")
plt.show()
...
for x in hhbuild:
    ...
    print(
        "Coins:",
        sim.getCoinBalance(x.address),
        "MoneyBal:",
        sim.getMoneyBalance(x.address),
    )
# print ("ADMCoins:", getCoinBalance(sp, spAccount), "ADMMoneyBal:",
# getMoneyBalance(sp, spAccount))
# print ("--------")
# Qtable
# for x in sp.Qtable:
# print (x, sp.Qtable[x])
...
print(
    "Carbon energy print: ",
    round(sp.totalOuterGrid / float(sp.totalOuterGrid + sp.totalSolarEnergy) * 100, 2),
    " %",
)
print(
    "Solar energy print: ",
    round(
        sp.totalSolarEnergy / float(sp.totalOuterGrid + sp.totalSolarEnergy) * 100, 2
    ),
    " %",
)
