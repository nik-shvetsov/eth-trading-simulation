...
class ServiceProvider(object):
    def __init__(self, id, address, addresses, households):
        self.id = id
    ...
    def splitVolumeInList(self, listoflists, divNum):# ["1",1.0, 200, 7] split into simple records
        newlist = []
    ...
    def loyaltyDA(self, buyListLoyal, sellListLoyal, divNum): #auction with loyalty price formation
        ...
        if (k > -1):
            ...
            while (True):
                ...
                for x in range(minServed):
                    ...
                    # form transaction
                    transactions.append((returnBuyList[x][0], returnSellList[x][0], divNum, newLoyalPrice))
            ...
            retList = [returnBuyList, returnSellList, cutSellList, cutBuyList, transactionsSummed, eqAuction]
    ...
    def simpleDReduceAuction(self, buyList, sellList):
        ...
        sellList.sort(key=lambda x: (x[2], -x[1])) #higher amounts served first
        ...
        if (k > -1): 
                ...
                for x in range(len(returnSellList)):
                    ...
                    for y in range(len(returnBuyList)):
                        ...
                        if (neededEnergy == sellingEnergy):
                            # form transaction
                            newAvgPrice = (returnBuyList[y][2] + returnSellList[x][2])/2
                            transactions.append((returnBuyList[y][0], returnSellList[x][0], sellingEnergy, newAvgPrice))
                            # move to next supplier/seller pair
                            returnSellList.remove(returnSellList[x])
                            ...
                            # form transaction
                            newAvgPrice = (returnBuyList[y][2] + returnSellList[x][2])/2
                            transactions.append((returnBuyList[y][0], returnSellList[x][0], neededEnergy, newAvgPrice))
                            remainsSeller = math.fabs(neededEnergy - sellingEnergy)
                            # update x[1] - update seller
                            newObj = (returnSellList[x][0], remainsSeller, returnSellList[x][2])
                            ...
                            newObj = (returnBuyList[y][0], remainsBuyer, returnBuyList[y][2])
            ...
            retList = [returnBuyList, returnSellList, cutSellList, cutBuyList, transactions, eqAuction]
